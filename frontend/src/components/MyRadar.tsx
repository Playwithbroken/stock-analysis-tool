...
export default function MyRadar({ onAnalyze, onOpenSignals }: MyRadarProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const [watchRes, historyRes, briefRes] = await Promise.all([
      ...
      <section className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
        <div className="surface-panel rounded-[2.5rem] p-6 sm:p-8">
          ...
          <p className="mt-5 max-w-2xl text-base leading-7 text-slate-600 sm:text-lg">
            Statt generischer Discovery startet die App jetzt bei deiner Watchlist:
            Insider, Congress-Filings, Berkshire-Signale, Morning Brief und persönlicher Alert-History.
          </p>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <button
              ...
              className="rounded-[1.2rem] bg-[#101114] px-5 py-3 text-xs font-extrabold uppercase tracking-[0.18em] text-white"
            >
              Open Signals
            </button>
            {topTicker?.ticker && (
        ...
        <div className="space-y-4">
          <div className="surface-strong rounded-[2.5rem] p-6 sm:p-8">
            <div className="text-[11px] font-extrabold uppercase tracking-[0.22em] text-white/50">
              Daily Brief
            </div>
            <div className="mt-4 space-y-3">
              <div className="rounded-[1.4rem] border border-white/10 bg-white/5 p-4 text-sm text-white/85">
                {topTicker?.events?.[0]
            ...
            <div className="mt-4 space-y-3">
              {history.length ? (
                ...
                <div className="text-sm text-slate-500">
                  Noch keine gesendeten Alert-History-Einträge.
                </div>
              )}
            </div>
          </div>
