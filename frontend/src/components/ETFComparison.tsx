      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div className="surface-panel rounded-[1.8rem] p-5">
          <div className="mb-4 flex items-center justify-between">
            ...
            <span className="text-xs text-slate-500 dark:text-slate-400">Nach Gewichtung</span>
          </div>
          ...
          <div className="space-y-2">
            {analysis.holdings.length > 0 ? (
              analysis.holdings.map((holding) => (
                <div
                  key={holding.symbol}
                  className="group relative flex cursor-help items-center justify-between rounded-xl border border-black/8 bg-white/78 p-3 transition-all hover:border-[var(--accent)]/20 hover:bg-white dark:border-white/10 dark:bg-white/5 dark:hover:border-white/20 dark:hover:bg-white/10"
                >
                  <div className="min-w-0">
                    <span className="block truncate font-mono text-sm font-bold text-slate-900 dark:text-white">
                      {holding.symbol}
                    </span>
                  ...
                  <div className="invisible absolute left-full top-0 z-50 ml-4 w-64 translate-x-2 rounded-xl border border-black/8 bg-white p-4 opacity-0 shadow-[0_20px_40px_rgba(17,24,39,0.12)] transition-all delay-150 group-hover:visible group-hover:translate-x-0 group-hover:opacity-100 dark:border-white/10 dark:bg-[#1c1c1e] dark:shadow-[0_20px_40px_rgba(0,0,0,0.5)]">
                    ...
                    <div className="mb-3 text-xs font-medium text-slate-600 dark:text-slate-300">
                      {holding.name}
                    </div>
              ...
              <div className="rounded-xl border border-dashed border-black/8 bg-white/70 py-8 text-center text-slate-500 dark:border-white/10 dark:bg-white/5 dark:text-slate-400">
        ...
        <div className="surface-panel flex min-h-[300px] flex-col items-center justify-center rounded-[1.8rem] p-5">
          <h3 className="mb-6 self-start text-sm font-bold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">
          ...
          {analysis.holdings.length > 0 ? (
                    ...
                    <RechartsTooltip
                      content={({ active, payload }) => {
                        if (active && payload && payload.length) {
                          return (
                            <div className="rounded-lg border border-black/8 bg-white p-3 shadow-[0_18px_36px_rgba(17,24,39,0.1)] dark:border-white/10 dark:bg-[#1c1c1e] dark:shadow-[0_18px_36px_rgba(0,0,0,0.5)]">
                              <div className="font-bold text-[var(--accent)]">
                                {payload[0].name}
                              </div>
                              <div className="mb-1 text-[10px] text-slate-500 dark:text-slate-400">
            ...
            <div className="text-sm text-slate-500 dark:text-slate-400">Keine Allokationsdaten vorhanden.</div>
          )}
        </div>
      </div>
    </div>
...
function InfoCard({
  ...
  const toneClass =
    tone === "emerald"
      ? "bg-emerald-500/7 border-emerald-500/14 text-emerald-700 dark:text-emerald-400"
      : tone === "amber"
        ? "bg-amber-500/7 border-amber-500/14 text-amber-700 dark:text-amber-400"
        : tone === "blue"
          ? "bg-black/[0.03] border-black/8 text-slate-700 dark:bg-white/5 dark:border-white/10 dark:text-slate-200"
          : "bg-slate-500/7 border-slate-500/14 text-slate-700 dark:text-slate-300";
  ...
  return (
    <div className={`rounded-xl border p-4 ${toneClass}`}>
      ...
      <div className="truncate text-xl font-bold text-slate-900 dark:text-white">{value}</div>
